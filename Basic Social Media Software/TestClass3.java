...
public class TestClass3 {
    ...
    public static void main(String[] args){
        ...
        users[3].blockUser(users, "sibel");
        users[3].logout();

        users[1].login();
        users[1].printProfile(users, "gizem");
        users[1].sendMessage(users, 1, 3, null, 0);
        ...
        users[1].logout();
        users[2].login();
        users[2].printProfile(users, "zeki"); //user not found error
        users[2].viewInteractions(users, "ali"); //user not found error
        users[2].printPosts(users, "ali"); //user not found error
        users[2].blockUser(users, "ahmet"); //user not found error
        users[2].sendMessage(users, 2, 2, null, 0); //cannot message yourself error
        users[2].sendMessage(users, 1, 2, null, 0); //sender must be current user error
        users[2].follow(users, "gokhan"); //can not follow yourself error
        */
    }
